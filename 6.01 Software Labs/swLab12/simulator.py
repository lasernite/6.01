...
class Simulator(Tk):
    ...
    def __init__(self, model, ideal):
        ...
        self._ugly_hidden_init(model, ideal)

    def _ugly_hidden_init(self, model, ideal):
        # hmm is an instance of 
        self.model = model
        self.hmmSim = model.makeSimulator()
        self.se = model.makeStateEstimator()
        self.hmmSim.initialize()
        self.se.reset()
        self.ideal = ideal
    ...
    def _setupUI(self):
        ...
        self.worldLabel = Label(self, text='World:', bg=bgColor)
        ...
        self.idealMarks = [Label(self, textvariable=i,  bg=bgColor) for i in self.idealVars]
        self.idealLabel = Label(self, text='Ideal Readings:',  bg=bgColor)
        ...
        self.obsLabel = Label(self, textvariable=self.obsVar,  bg=bgColor)
        ...
        self.obsMarks = [Label(self, textvariable=i,  bg=bgColor) for i in self.obsVars]
        for ix in xrange(len(self.obsMarks)):
            self.obsMarks[ix].grid(row=6, column=ix+2)

        #prior belief
        self.priorgraph = [Canvas(self, width=self._CELL_SIZE,
                                  height=self._CELL_SIZE) \
                           for i in xrange(self.numStates)]
        self.priorLabel = Label(self, text='Prior: ',  bg=bgColor)
        ...
        self.priorMarks = [Label(self, textvariable=i,  bg=bgColor) for i in self.priorVars]
        for ix in xrange(len(self.priorMarks)):
            self.priorMarks[ix].grid(row=9, column=ix+2)
       
        #after transition update
        self.aftertransgraph = [Canvas(self, width=self._CELL_SIZE,
                                       height=self._CELL_SIZE) \
                                for i in xrange(self.numStates)]
        self.aftertransVar = StringVar()
        self.aftertransLabel = Label(self, textvariable=self.aftertransVar,  bg=bgColor)
        ...
        self.aftertransMarks = [Label(self, textvariable=i,  bg=bgColor) for i in self.aftertransVars]
        for ix in xrange(len(self.aftertransMarks)):
            self.aftertransMarks[ix].grid(row=15, column=ix+2)
        
        #after observation update
        self.posteriorgraph = [Canvas(self, width=self._CELL_SIZE,
                                      height=self._CELL_SIZE) \
                               for i in xrange(self.numStates)]
        self.posteriorVar = StringVar()
        self.posteriorLabel = Label(self, textvariable=self.posteriorVar,  bg=bgColor)
        ...
        self.posteriorMarks = [Label(self, textvariable=i,  bg=bgColor) for i in self.posteriorVars]
        for ix in xrange(len(self.posteriorMarks)):
            self.posteriorMarks[ix].grid(row=12, column=ix+2)

        Label(self, text='Actions: ',  bg=bgColor).grid(row=17,column=0)
        Button(self,text='Transition',command=self._step, bg=bgColor).\
                                              grid(row=17,column=2,columnspan=2)
        Button(self,text='Reset',command=self._reset,  bg=bgColor).\
                                              grid(row=17,column=4,columnspan=2)

        self.observation = StringVar()
        self.obsLabel = Label(self, textvariable=self.observation, height=1,  bg=bgColor)
    ...
    def _step(self):
        ...
        obs = self.hmmSim.makeObservation()
        self.hmmSim.makeTransition()
        self._drawWorld()
        self.observation.set('Observed %d' % obs)

        obsProbs = [self.model.observationDistribution(i).prob(obs) \
                         for i in xrange(self.numStates)]
        self._drawGraph(self.obsgraph,self.obsVars, obsProbs, color='green')
    ...
    def _drawGraph(self,graphvar,labelvar,values,color='blue'):
        '''
        ...
        for ((box,label),val) in zip(zip(graphvar,labelvar),values):
            ...
            box.create_rectangle(0,
                                 ...
                                 self._CELL_SIZE,
                                 self._CELL_SIZE, fill=color, outline=color)
            label.set('%.04f' % val)
    ...
    def _reset(self):
        ...
        self._ugly_hidden_init(self.model, self.ideal)
        self.update_idletasks()
